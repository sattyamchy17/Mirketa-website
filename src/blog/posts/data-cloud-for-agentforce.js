import featuredImage from "../../assets/blog/data-cloud-for-agentforce/agentforce_data_cloud_hero_banner.png";
...
export const post = {
  ...
  featuredImage,
  featuredImageAlt: "A balance scale weighing the cost of Salesforce Data Cloud against its benefits for Agentforce, labeled unified data, real-time context, licensing, and setup effort",
  ...
  content: [
    {
      type: "paragraph",
      text: "So here's a question we get constantly: do you actually need Data Cloud to run Agentforce, or is that just Salesforce padding the invoice? Fair question. The two get sold together so often that most people just assume it's one purchase. It isn't. Below is the honest version — what Data Cloud does, when it's worth the money, what it actually costs right now in 2026, and where you can probably skip it, at least for a while.",
    },
    { type: "heading2", text: "What Agentforce Actually Does" },
    {
      type: "paragraph",
      text: "[Agentforce](/agentforce) builds AI agents that live inside your Salesforce org. Case resolution, lead qualification, order lookups, that kind of thing. The pitch is that these agents don't just answer questions, they act — update a record, kick off a workflow, hand things off to a human when they hit a wall.",
    },
    {
      type: "paragraph",
      text: "Nobody puts this on the pricing page, but here's the catch. Out of the box, an agent can only see what's already sitting in the Salesforce record right in front of it. Fine if the task is simple. Not so fine once you realize most customer relationships are scattered across way more than one system.",
    },
    { type: "heading2", text: "Where Data Cloud Fits In" },
    {
      type: "paragraph",
      text: "Data Cloud got renamed Data 360 sometime in late 2025, so if you see both terms floating around in different articles, that's why — same product. Its job is pulling data out of [Sales Cloud](/salesforce/sales-cloud), [Service Cloud](/salesforce/service-cloud), [Marketing Cloud](/salesforce/marketing-cloud), plus whatever else you're running (ERP, support desk, a spreadsheet someone's been maintaining since 2019) and mashing it into one profile per customer that actually stays current.",
    },
    {
      type: "paragraph",
      text: "What that means for Agentforce: the agent stops guessing off a partial record and starts working off the real picture. Past orders. Open tickets. Whether they clicked the last email campaign. Here's roughly how it fits together.",
    },
    {
      type: "callout",
      text: "Figure 1. Data Cloud unifies fragmented data before Agentforce reasons and acts on it.",
    },
    { type: "heading2", text: "Do You Actually Need It?" },
    {
      type: "paragraph",
      text: "Depends what you're building and where your data already lives, honestly. There's no universal answer here, so use the table below as a gut check, not a rulebook.",
    },
    {
      type: "table",
    ...
    {
      type: "paragraph",
      text: "Here's where budgets go sideways. Agentforce's own pricing looks fine when you first see it — a couple bucks a conversation, or credits you buy in bulk. Then Data Cloud lands on the same quote and suddenly what was a pilot budget is a six-figure ask. It happens almost every time.",
    },
    {
      type: "table",
    ...
    { type: "heading2", text: "So Why Do Teams Pay for It?" },
    {
      ...
      style: "bullet",
      items: [
        "**It keeps the agent honest.** Working off a unified profile instead of a partial record means fewer made-up answers and less stale info reaching the customer. Not zero, but noticeably fewer.",
        "**It sees across channels.** An agent handling a sales question can notice there's an open support ticket. Or catch that someone just got a marketing email before deciding how urgent their case is. None of that shows up if the data's still siloed.",
        "**Less gets punted to a human.** More context up front, more issues closed without a handoff. And when it does escalate, the human isn't starting from zero either.",
        "**You're not just buying it for this.** Whatever you build for Agentforce, the same unified profiles end up feeding personalization and whatever comes after this project.",
    ...
    { type: "heading2", text: "A Few Things Worth Doing First" },
    {
      ...
      style: "number",
      items: [
        "**Map your data sources.** Every system holding info the agent might need, and whether it's already talking to Salesforce or sitting off on its own.",
        "**Check if you've already paid for this.** Data Cloud running elsewhere in the org for CDP or personalization work means Agentforce is incremental cost, not a new line item.",
